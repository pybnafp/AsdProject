...
class GraphExtractor:
    ...
    async def _process_document(  # 利用chatmodel获得实体和关系
    ...
    async def _process_results(  # 将实体和关系添加到图中
        self,
        ...
        for source_doc_id, extracted_data in results.items():
            ...
            for record in records:
                ...
                if record_attributes[0] == '"entity"' and len(record_attributes) >= 4:
                    ...
                    if entity_name in graph.nodes(): # 如果实体已经存在，则增加描述description和来源id和实体类型entity_type
                ...
                if (
                    ...
                    if source not in graph.nodes(): # 如果source不存在，则添加为节点
                        graph.add_node(
                            source,
                            type="",
                            description="",
                    ...
                    if target not in graph.nodes(): # 如果target不存在，则添加为节点
                        graph.add_node(
                            target,
                            type="",
                            description="",
                    ...
                    if graph.has_edge(source, target): # 如果source和target之间存在边
                        ...
                        # 更新已有边的属性，增加权重weight和描述description和来源id
