...
def _merge_entities(entity_dfs) -> pd.DataFrame:
    if not entity_dfs:
        return pd.DataFrame(columns=["title", "type", "description", "text_unit_ids", "frequency"])  # type: ignore[list-item]
    all_entities = pd.concat(entity_dfs, ignore_index=True)
    # Ensure required columns exist even if upstream returns partial schema
    for required_col in ["title", "type", "description", "source_id"]:
        if required_col not in all_entities.columns:
            all_entities[required_col] = [] if required_col in ("description",) else None
    return (
