...
async def run_pipeline(
    ...
    # Ensure storages are resolved relative to config.root_dir for file-based paths
    input_storage = create_storage_from_config(config.input.storage, root_dir)
    output_storage = create_storage_from_config(config.output, root_dir)
    cache = create_cache_from_config(config.cache, root_dir)

    # load existing state in case any workflows are stateful
    state_json = await output_storage.get("context.json")  # 为None
    state = json.loads(state_json) if state_json else {}  # 为空字典
    ...
    if is_update_run:
        ...
        update_storage = create_storage_from_config(config.update_index_output, root_dir)
        ...
        logger.info("Copying previous output from %s to %s", output_storage._root_dir, previous_storage._root_dir)  # type: ignore[attr-defined]
...
async def _copy_previous_output(
    ...
    # Prefer reading previous datasets by known logical table names to avoid path/regex issues
    known_tables = [
        "documents",
        "entities",
        "relationships",
    ...
    for name in known_tables:
        try:
            table = await load_table_from_storage(name, storage)
            await write_table_to_storage(table, name, copy_storage)
            logger.info("Copied table %s", name)
            copied_any = True
        except Exception:
            # skip missing tables quietly
            continue

    if copied_any:
        return

    # Fallback: discover by parquet suffix if known names weren't found
    files_iter = list(storage.find(re.compile(r"\.parquet$")))
    if len(files_iter) == 0:
        logger.warning("No previous parquet files found to copy from %s", getattr(storage, "_root_dir", "<unknown>"))
        return
