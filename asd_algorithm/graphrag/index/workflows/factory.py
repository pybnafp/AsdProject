...
class PipelineFactory:
    ...
    def create_pipeline(
        ...
        # Normalize method to enum value (string) for dict key lookup
        normalized_method = (
            method.value if isinstance(method, IndexingMethod) else str(method)
        )
        workflows = config.workflows or cls.pipelines.get(normalized_method, [])
