...
def create_storage_from_config(output: StorageConfig, root_dir: str | None = None) -> PipelineStorage:
    """Create a storage object from the config.

    If storage type is file and base_dir is relative, join with root_dir when provided.
    """
    storage_config = output.model_dump()

    # Normalize file paths for file-based storage
    if (
        root_dir
        and storage_config.get("type") == StorageType.file.value
        and "base_dir" in storage_config
    ):
        base_dir = Path(storage_config["base_dir"])
        if not base_dir.is_absolute():
            storage_config["base_dir"] = str(Path(root_dir) / base_dir)
