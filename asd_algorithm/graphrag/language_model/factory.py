...
# 移除 DashScope 导入以避免循环导入
from graphrag.language_model.providers.dashscope.models import DashScopeChatModel
from graphrag.language_model.providers.local.models import LocalQwen3ChatModel
...
# 移除 DashScope 注册以避免循环导入
ModelFactory.register_chat(
    "dashscope", lambda **kwargs: DashScopeChatModel(**kwargs)
)
# 注册本地 Qwen3 模型
ModelFactory.register_chat(
    "local_qwen3", lambda **kwargs: LocalQwen3ChatModel(**kwargs)
)
