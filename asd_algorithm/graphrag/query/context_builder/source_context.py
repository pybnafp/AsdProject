...
def count_relationships(
    ...
    if text_unit.relationship_ids is None or (hasattr(text_unit.relationship_ids, '__len__') and len(text_unit.relationship_ids) == 0):
        # Use list comprehension to count relationships where the text_unit.id is in rel.text_unit_ids
        return sum(
            1
            for rel in entity_relationships
            if rel.text_unit_ids is not None and (not hasattr(rel.text_unit_ids, '__len__') or len(rel.text_unit_ids) > 0) and text_unit.id in rel.text_unit_ids
        )
