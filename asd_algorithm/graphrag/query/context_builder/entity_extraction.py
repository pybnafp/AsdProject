...
def map_query_to_relationships(
    query: str,
    relationship_embedding_vectorstore: BaseVectorStore,
    text_embedder: EmbeddingModel,
    all_relationships: list[Relationship],
    k: int = 10,
) -> list[Relationship]:
    """Extract relationships that match a given query using semantic similarity of text embeddings of query and relationship descriptions."""
    ...
    if query:
        # 以关系描述为向量检索对象
        search_results = relationship_embedding_vectorstore.similarity_search_by_text(
            text=query,
            text_embedder=lambda t: text_embedder.embed(t),
            k=k,
        )
        # 结果中的document.id 对应关系表中的 id（或 short_id），收集(relationship, score)
        id_to_rel = {rel.id: rel for rel in all_relationships if isinstance(rel.id, str)}
        short_to_rel = {getattr(rel, "short_id", None): rel for rel in all_relationships if getattr(rel, "short_id", None)}
        rel_with_scores: list[tuple[Relationship, float]] = []
        for result in search_results:
            doc_id = getattr(result.document, "id", None)
            rel_obj = id_to_rel.get(doc_id) or short_to_rel.get(doc_id)
            if rel_obj is not None:
                score = float(getattr(result, "score", 0.0) or 0.0)
                rel_with_scores.append((rel_obj, score))
        # 去重并按分数降序排序
        seen = set()
        unique_rel_with_scores: list[tuple[Relationship, float]] = []
        for rel_obj, score in rel_with_scores:
            print(f"rel_obj: {rel_obj}, score: {score}")
            key = getattr(rel_obj, "id", None) or getattr(rel_obj, "short_id", None) or id(rel_obj)
            if key in seen:
                continue
            seen.add(key)
            unique_rel_with_scores.append((rel_obj, score))
        unique_rel_with_scores.sort(key=lambda x: x[1], reverse=True)
        matched_relationships = [rel for rel, _ in unique_rel_with_scores[:k]]
    else:
        # 空查询按 rank 降序取前 k
        sorted_rels = sorted(all_relationships, key=lambda r: (r.rank or 0), reverse=True)
        matched_relationships = sorted_rels[:k]
