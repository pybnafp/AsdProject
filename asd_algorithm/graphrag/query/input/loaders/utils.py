...
def to_optional_list(
    ...
    if item_type is not None:
        # 过滤掉None值，只保留指定类型的项目
        filtered_value = [v for v in value if v is not None and isinstance(v, item_type)]
        if len(filtered_value) != len(value):
            # 如果有None值被过滤掉，记录警告但不抛出错误
            import logging
            logger = logging.getLogger(__name__)
            logger.warning(f"Filtered out {len(value) - len(filtered_value)} None values from {column_name}")
...
def to_int(data: Mapping[str, Any], column_name: str | None) -> int:
    ...
    if isinstance(value, float):
        # 检查是否为NaN
        if pd.isna(value):
            msg = f"value is NaN: {value} ({type(value)})"
            raise ValueError(msg)
        value = int(value)
