...
class GlobalSearch(BaseSearch[GlobalContextBuilder]):
    ...
    async def stream_search(
        ...
        context_result = await self.context_builder.build_context(  # 非none
            query=query,
            conversation_history=conversation_history,
            **self.context_builder_params,
        )
        # 将query与所用community_reports输出到日志（stream 模式-全局）
        try:
            ...
            community_reports_df = records.get("reports")
            
            # 记录社区报告信息
            community_reports_list: list[str] = []
            if isinstance(community_reports_df, pd.DataFrame) and not community_reports_df.empty:
                if "title" in community_reports_df.columns:
                    community_reports_list = community_reports_df["title"].astype(str).tolist()

            timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            with log_file.open("a", encoding="utf-8") as f:
                f.write(f"[{timestamp}] mode=global(stream)\n")
                f.write(f"query: {query}\n")
                f.write("entities: (none) - Global search uses community reports\n")
                f.write("relationships: (none) - Global search uses community reports\n")
                
                if community_reports_list:
                    f.write("community_reports: " + ", ".join(community_reports_list[:10]) + "\n")  # 限制显示数量
                else:
                    f.write("community_reports: (none)\n")

                # 详细记录社区报告 - 使用实际的数据列
                if isinstance(community_reports_df, pd.DataFrame) and not community_reports_df.empty:
                    f.write("community_reports_detail:\n")
                    for _, row in community_reports_df.iterrows():
                        hrid = str(row.get("id", ""))
                        title = str(row.get("title", ""))
                        occurrence = str(row.get("occurrence", ""))
                        weight = str(row.get("weight", ""))
                        rank = str(row.get("rank", ""))
                        # 截取full_content的前100个字符
                        full_content = str(row.get("full_content", ""))[:100]
                        f.write(f"- id={hrid} | title={title} | occurrence={occurrence} | weight={weight} | rank={rank} | content={full_content}...\n")
                else:
                    f.write("community_reports_detail: (none)\n")
                f.write("-" * 60 + "\n")
        except Exception:
            pass
    ...
    async def search(
        ...
        **kwargs: Any,
        ...
        # 将query与所用community_reports输出到日志
        try:
            log_dir = Path("ragtest_test/logs")
            log_dir.mkdir(parents=True, exist_ok=True)
            log_file = log_dir / "query.log"
            ...
            timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            with log_file.open("a", encoding="utf-8") as f:
                f.write(f"[{timestamp}] mode=global\n")
                f.write(f"query: {query}\n")
                f.write("entities: (none) - Global search uses community reports\n")
                f.write("relationships: (none) - Global search uses community reports\n")
        ...
        map_responses = await asyncio.gather(*[
            self._map_response_single_batch(  # 利用llm生成response
                context_data=data,
                query=query,
                max_length=self.map_max_length,
                **self.map_llm_params,
