...
def get_dashscope_model_parameters_from_config(config) -> dict:
    """Get DashScope model parameters from config."""
    try:
        # 读取配置文件
        config_parser = configparser.ConfigParser()
        config_parser.read('config.ini')
        
        dashscope_config = config_parser['dashscope']
        
        return {
            'api_key': dashscope_config.get('api_key'),
            'model': dashscope_config.get('model', 'qwen-plus'),
            'max_tokens': int(dashscope_config.get('max_tokens', 1024)),
        }
    except Exception as e:
        print(f"Warning: Could not load DashScope config: {e}")
        return {
            'api_key': None,
            'model': 'qwen-plus',
            'max_tokens': 1024,
...
def get_local_search_engine(
    ...
    response_type: str,
    entity_embedding_store: BaseVectorStore,
    relationship_embedding_store: BaseVectorStore,
    ...
    # 检查是否使用 DashScope 模型
    if model_settings.type == "dashscope":
        # 延迟导入以避免循环导入
        from graphrag.language_model.providers.dashscope.models import DashScopeChatModel
        dashscope_params = get_dashscope_model_parameters_from_config(config)
        # 获取系统提示词
        system_prompt = system_prompt or "你是一个专业的AI助手，请根据提供的信息回答问题。"
        
        # 提升DashScope的生成上限，避免输出被截断
        max_tokens = max(dashscope_params['max_tokens'] or 0, 4096)
        chat_model = DashScopeChatModel(
            name="local_search_chat",
            config=model_settings,
            api_key=dashscope_params['api_key'],
            model=dashscope_params['model'],
    ...
    # 根据模型类型获取参数
    if model_settings.type == "dashscope":
        model_params = {}  # DashScope 模型不需要额外的参数
    else:
    ...
    return LocalSearch(
        ...
        context_builder=LocalSearchMixedContext(
            ...
            relationship_text_embeddings=relationship_embedding_store,
        ...
        context_builder_params={
            ...
            # 将映射模式切换为 relationship，使local search以关系为核心构建上下文
            "map_by": "relationship",
...
def get_global_search_engine(
    ...
    if model_settings.type == "dashscope":
        ...
        from graphrag.language_model.providers.dashscope.models import DashScopeChatModel
        dashscope_params = get_dashscope_model_parameters_from_config(config)
        # 获取系统提示词（全局搜索使用 map 提示词）
        system_prompt = map_system_prompt or "你是一个专业的AI助手，请根据提供的信息回答问题。"
...
def get_basic_search_engine(
    ...
    # 检查是否使用 DashScope 模型
    if chat_model_settings.type == "dashscope":
        # 延迟导入以避免循环导入
        from graphrag.language_model.providers.dashscope.models import DashScopeChatModel
        dashscope_params = get_dashscope_model_parameters_from_config(config)
        # 获取系统提示词
