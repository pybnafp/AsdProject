...
def read_indexer_reports(
    ...
    if not dynamic_community_selection:
        ...
        # 用内连接（inner）将 reports_df 与 filtered_community_df 关联：
        # 只保留 reports_df 中 “属于最高层级社区” 的报告，剔除低层级冗余报告。
        reports_df = reports_df.merge(
            filtered_community_df, on="community", how="inner"
        )
    # 为社区报告的full_content生成嵌入向量content_embedding_col
    if config and (
...
def read_indexer_entities(
    ...
    nodes_df = nodes_df.loc[:, ["id", "community"]] # 只保留nodes_df的id和community列,聚焦实体与社区的关联关系
    nodes_df["community"] = nodes_df["community"].fillna(-1)  # -1 通常作为 “无社区” 的标记
    ...
    # 为每个实体补充回名称（title）、描述（description）、度数（degree）等原始信息
    final_df = nodes_df.merge(entities, on="id", how="inner").drop_duplicates(
        subset=["id"]
    )
    # 最终 final_df 表：每个实体一行，包含完整实体信息 + 去重并格式化的社区 ID 列表
