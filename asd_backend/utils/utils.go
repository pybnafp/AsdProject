...
func AppDebug() bool {
	// 获取配置实例
	return conf.CONFIG.SystemConfig.Debug
}
...
func GetImageUrl(path string) string {
	return conf.CONFIG.SystemConfig.Image + path
}
...
func SaveImage(url string, dirname string) (string, error) {
	...
	// 判断是否本站图片
	if gstr.Contains(url, conf.CONFIG.SystemConfig.Image) {
		// 本站图片
		...
		if gstr.Contains(url, "temp") {
			...
			// 原始图片地址
			oldPath := gstr.Replace(url, conf.CONFIG.SystemConfig.Image, conf.CONFIG.Attachment.FilePath)
			// 目标目录地址
			newPath := ImagePath() + "/" + dirname + gstr.Replace(url, conf.CONFIG.SystemConfig.Image+"/temp", "")
			// 移动文件
			os.Rename(oldPath, newPath)
			return gstr.Replace(newPath, conf.CONFIG.Attachment.FilePath, ""), nil
		} else {
			// 非临时图片
			path := gstr.Replace(url, conf.CONFIG.SystemConfig.Image, "")
			return path, nil
		}
	} else {
		// 远程图片
...
// HandleDBUpdateError 处理数据库更新错误
func HandleDBUpdateError(rows int64, err error) error {
	if err != nil {
		return errors.New("更新失败: " + err.Error())
	}
	if rows == 0 {
		return errors.New("更新失败: 未找到记录或数据未变更")
	}
	return nil
}

func HandleDBDeleteError(rows int64, err error) error {
	if err != nil {
		return errors.New("删除失败: " + err.Error())
	}
	if rows == 0 {
		return errors.New("删除失败: 未找到记录")
	}
	return nil
}

func GetUserIDKey(userId int) string {
	IDKey, _ := Md5("xnxn" + gconv.String(userId))
	return IDKey[:6]
}
